...
treated_core_data <- treated_unit %>%
  st_drop_geometry() %>% select(starts_with("Y"))
...
ggplot() +
  ...
  # 2. Control Units (파란색)
  geom_sf(
    data = filter(final_sf, type == "Control Unit"),
...
ggplot() +
  ...
  geom_sf(
    data = candidates %>% filter(assigned_band != "Outside"),
  ...
  geom_sf(
    data = st_centroid(candidates %>% filter(assigned_band != "Outside")),
    size = 0.5, color = "black"
  ) +
  scale_fill_brewer(palette = "Set2") +
  theme_minimal() +
  labs(
    title = "지역 중심점(Centroid) 기준 밴드 할당 결과",
    fill = "Distance Band"
  )
```
...
if (nrow(final_controls) > 0) {
  ...
  ggplot() +
    ...
    geom_sf(
      data = gdf %>% filter(sgg_code %in% excluded_sggs),
    ...
    labs(
      title = "Treated Exclusion vs Control Centers",
